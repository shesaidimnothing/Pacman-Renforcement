...
export const createScoreManager = () => {
  ...
  return {
    ...
    calculateDotScore: (): number => SCORE_PER_DOT,
    
    calculateVictoryBonus: (): number => VICTORY_BONUS
  };
};
...
export const loadGameData = async (): Promise<{ highScore: number; lastScore: number }> => {
  try {
...
export const saveLastScore = (score: number): void => {
  try {
...
export const getLastScore = (): number => {
  try {
...
export const initializeScoring = async (): Promise<{
  highScore: number;
  lastScore: number;
...
const checkFirstVisit = (): Promise<boolean> => {
...
export const saveScoreWithTimeout = (score: number, timeout: number = 5000): Promise<boolean> => {
